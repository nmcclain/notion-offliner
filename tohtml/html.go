...
func (c *Converter) renderPageHeader(block *notionapi.Block) {
	c.Printf(`<header>`)
	{
		...
		c.Printf(`<h1 class="page-title">`)
		{
			...
			// log.Printf("block.InlineContent = %+v\n", block.InlineContent)
			if len(block.InlineContent) > 0 {
				// log.Printf("block.InlineContent[]0 = %+v\n", block.InlineContent[0])
			}
			// log.Printf("block = %+v\n", block)
		}
		c.Printf(`</h1>`)
...
func (c *Converter) RenderCollectionViewPage(block *notionapi.Block) {
	...
	c.Printf(`<figure id="%s" class="link-to-page">`, block.ID)
	{
		...
		c.Printf(`<a href="%s">`, filePath)
		{
			...
			c.Printf(`<span class="icon">%s</span>`, icon)
		}
...
func (c *Converter) RenderFile(block *notionapi.Block) {
	...
	{
		c.Printf(`<div class="source">`)
		c.Printf(`<svg viewBox="0 0 30 30" class="file" style="width: 22px; height: 22px; display: inline; fill: inherit; flex-shrink: 0; backface-visibility: hidden;"><path d="M22,8v12c0,3.866-3.134,7-7,7s-7-3.134-7-7V8c0-2.762,2.238-5,5-5s5,2.238,5,5v12c0,1.657-1.343,3-3,3s-3-1.343-3-3V8h-2v12c0,2.762,2.238,5,5,5s5-2.238,5-5V8c0-3.866-3.134-7-7-7S6,4.134,6,8v12c0,4.971,4.029,9,9,9s9-4.029,9-9V8H22z"></path></svg> `)
		{
			uri := GetDownloadedFileName(block.Source, block)
			c.A(uri, block.Title, "")
		}
		c.Printf(` <span style="font-size: 12px; color: rgba(55, 53, 47, 0.6); margin-left: 6px;">%s</span>`, block.FileSize)
		c.Printf(`</div>`)
		c.RenderCaption(block)
	}
	c.Printf(`</figure>`)
...
// RenderCrumbs alternate breadcrumb renderer
func (c *Converter) RenderCrumbs(crumbs []Crumb, title string) {
	if c.NotionCompat {
	...
	c.Printf(`<div class="block-color-gray_background breadcrumbs sans">`)
	// TODO: add icon

	sub := ""
	for j := 0; j < len(crumbs); j++ {
		sub = "../" + sub
	}
	...
	for _, crumb := range crumbs {
		...
		if crumb.Skip {
			c.Printf(`<span>%s</span>`, name)
		} else {
			c.Printf(`<span><a href="%s">%s</a></span>`, EscapeHTML(sub+crumb.Link), name)
		}
		c.Printf("<span> | </span>")
	}
	c.Printf(`<span>%s</span>`, title)
	c.Printf(`</div>`)
}
...
func (c *Converter) renderTableCell(tv *notionapi.TableView, row, col int) {
	...
	// log.Printf("renderTableCell: colVal %s", colVal)
	...
	if typ == notionapi.ColumnTypeTitle {
		...
		if isEmptyBlock(rowPage) {
			...
			uri := ""
			if len(tv.Page.Root().Title) > 1 {
				uri = safeName(tv.Page.Root().Title) + "/" + c.tableTitleCellURL(tv, row, col)
			} else {
...
func (c *Converter) renderTableRow(tv *notionapi.TableView, row int) {
	tr := tv.Rows[row]
	// log.Printf("renderTableRow tr = %+v\n", tr)
	...
	} else { // sometimes lists have no columns
		t := tv.Rows[row].Page.Properties["title"].([]interface{})[0].([]interface{})[0].(string)
		colVal := fmt.Sprintf(`%s`, t)
		if !isEmptyBlock(tv.Rows[row].Page) {
			uri := ""
			if len(tv.Page.Root().Title) > 1 {
				uri = safeName(tv.Page.Root().Title) + "/" + safeName(tv.Collection.GetName()) + "/" + safeName(t) + ".html"
			} else {
				uri = safeName(tv.Collection.GetName()) + "/" + safeName(t) + ".html"
			}
			colVal = fmt.Sprintf(`<a href="%s">%s</a>`, uri, t)
		}
		c.Printf(`<td class="cell-%s">%s</td>`, "TK", colVal)
	}
	c.Printf("</tr>\n")
}
...
func (c *Converter) RenderCollectionView(block *notionapi.Block) {
	...
	var tv *notionapi.TableView
	found := false
	for _, tv = range block.TableViews {
		// log.Printf("tv = %+v\n", tv)
		if tv.CollectionView.Type == notionapi.CollectionViewTypeList || tv.CollectionView.Type == notionapi.CollectionViewTypeTable {
			found = true
			break
		}
	}
	if !found {
		log.Printf("couldn't find a table view we can render for block '%s'\n", tv.CollectionView.ID)
	...
	c.Printf(`<div id="%s" class="collection-content">`, block.ID)
	{
		...
		if !isList {
			...
			{
				c.Printf(`<tr>`)
				for col := 0; col < tv.ColumnCount(); col++ {
...
func (c *Converter) ToHTML(crumbs []Crumb) ([]byte, error) {
	if c.NotionCompat {
	...
	c.PushNewBuffer()
	c.RenderCrumbs(crumbs, c.Page.Root().Title)
